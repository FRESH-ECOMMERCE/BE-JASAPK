...
const requestOtp = async (req, res) => {
    const { error } = (0, validateRequest_1.validateRequest)(
        userSchema_1.requestOtpSchema,
        req.body
    );
    if (error != null) return (0, handleValidaionError_1.default)(res, error);
    ...
    try {
        ...
        if (requestBody.otpType === 'resetPassword' && existingUser === null) {
            ...
            return res
                ...
                .json(response_1.ResponseData.error(message));
        }
        ...
        const wablasResponse = await axios.get(
            `${waBlasSettings.waBlasServer}/send-message?phone=${requestBody.whatsappNumber}&message=${message}&token=${waBlasSettings.waBlasToken}`
        );
